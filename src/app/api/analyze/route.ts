...
import { validateScorecard, computeVerdict, METHODOLOGY_VERSION } from "@/lib/verdict-engine";
import { rateLimit, rateLimited } from "@/lib/rate-limit";
import { upsertVerifiedProduct, slugify } from "@/lib/verified-store";
import type { ClaimCheckResult, Scorecard, FinalVerdict } from "@/lib/types";
...
// Version prefix invalidates any cached result produced by an older engine —
// pre-claims-framework scorecards can never be served again.
...
// Compact claim findings for prompt injection + code-side validation
function compactClaimFindings(cf: ClaimCheckResult | null | undefined) {
...
// Only products passing ALL THREE GATES of The Clean Sheet Standard join the registry
function maybeRegisterVerified(card: Scorecard, verdict: FinalVerdict, claimFindings: ReturnType<typeof compactClaimFindings>) {
  try {
    ...
    upsertVerifiedProduct({
      ...
      integrityScore: claimFindings?.integrityScore ?? null,
      imageUrl: claimFindings?.imageUrl ?? null,
      summary: card.summary || "",
      usageGuidance: card.usageGuidance ?? null,
      verifiedAt: new Date().toISOString(),
      methodologyVersion: METHODOLOGY_VERSION,
    });
  } catch { /* registry failure must never break the review */ }
}
...
export async function POST(req: Request) {
  try {
    ...
    const claimFindings = compactClaimFindings(rawClaimFindings);
...
${JSON.stringify(claimFindings.claims.map((c) => ({ claim: c.claim, verdict: c.verdict, evidenceLevel: c.evidenceLevel, explanation: c.explanation })))}
` : ""}
...
${scrapedContext}`;
      ...
      prompt = isComparison
        ...
        : isExpert
          ? q
          : `Analyze this product: ${q}${claimFindings ? `
...
${JSON.stringify(claimFindings.claims.map((c) => ({ claim: c.claim, verdict: c.verdict, evidenceLevel: c.evidenceLevel, explanation: c.explanation })))}` : ""}`;
    ...
    if (urlInput && (!parsed || parsed.type === "out_of_scope" || !isValidScorecard(parsed as Record<string, unknown>))) {
      ...
      if (fallbackQuery) {
        const fallbackText = await generateResilient(systemInstruction, `Analyze this beauty/cosmetic product (this is definitely a beauty product — lipstick, skincare, makeup, personal care — do NOT return out_of_scope): ${fallbackQuery}`);
        ...
        if (fallbackParsed && fallbackParsed.type !== "out_of_scope" && isValidScorecard(fallbackParsed as Record<string, unknown>)) {
          const validated = validateScorecard(fallbackParsed as Scorecard, claimFindings);
          const verdict = computeVerdict(validated, claimFindings);
          maybeRegisterVerified(validated, verdict, claimFindings);
          const body = { type: "single", scorecard: validated, verdict };
    ...
    if (isComparison && finalParsed.type === "comparison" && finalParsed.productA && finalParsed.productB) {
      finalParsed.productA = validateScorecard(finalParsed.productA as Scorecard);
      finalParsed.productB = validateScorecard(finalParsed.productB as Scorecard);
    ...
    const validated = validateScorecard(finalParsed as Scorecard, claimFindings);
    const verdict = computeVerdict(validated, claimFindings);
    maybeRegisterVerified(validated, verdict, claimFindings);
    const body = { type: "single", scorecard: validated, verdict };
    RESULT_CACHE.set(cacheKey, body);
