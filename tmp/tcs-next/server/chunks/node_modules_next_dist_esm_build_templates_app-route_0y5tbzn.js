module.exports=[15359,e=>{"use strict";var t=e.i(47909),i=e.i(74017),a=e.i(96250),r=e.i(59756),n=e.i(61916),o=e.i(74677),s=e.i(69741),l=e.i(16795),c=e.i(87718),d=e.i(95169),u=e.i(47587),m=e.i(66012),p=e.i(70101),h=e.i(26937),g=e.i(10372),f=e.i(93695);e.i(20232);var y=e.i(5232);let v=`
...
Step 2 — Map prices across all Indian platforms.
  ...
  Calculate price per ml or per g = price \xf7 quantity.
...
`;var b=e.i(71994),w=e.i(68238);async function I(e,t=6e4){try{let i=await fetch("https://r.jina.ai/"+e,{headers:{Accept:"text/plain","X-Return-Format":"markdown"},signal:AbortSignal.timeout(15e3)});if(!i.ok)return null;return(await i.text()).slice(0,t)}catch{return null}}function C(e,t=9e3){let i=e.split("\n"),a=i.findIndex(e=>/^#\s+\S/.test(e.trim()));return -1===a&&(a=0),i.slice(a).map(e=>{let t=e.trim();if(/^!\[/.test(t)){let e=t.match(/^!\[([^\]]*)\]/)?.[1]?.replace(/^Image \d+:\s*/i,"").trim();return e&&e.length>3?`[Graphic on page: ${e}]`:""}return e}).filter(e=>{let t=e.trim();return!!t&&(!((t.match(/\]\(/g)||[]).length>=3)||!(t.replace(/\[[^\]]*\]\([^)]*\)/g,"").trim().length<10))}).join("\n").slice(0,t)}let S=/report|study|clinical|certificate|certif|test(?:ed|ing)?|proof|evidence|dermat/i;function N(e){return(e.toLowerCase().match(/[a-z0-9]+/g)||[]).filter(e=>e.length>1)}let A=new Set(["the","and","for","with","new","pro","our"]);function R(e){return e.toLowerCase().replace(/\s+\d+$/,"").replace(/[^a-z0-9]+/g," ").trim()}async function k(e){let t={chosen:null,distinct:[],ambiguous:!1},i=e.trim();if(!i)return t;let a=await I(`https://incidecoder.com/search?query=${encodeURIComponent(i)}`);if(!a)return t;let r=[...a.matchAll(/\[([^\]]+)\]\((?:https:\/\/incidecoder\.com)?\/products\/([a-z0-9-]+)\)/gi)].map(e=>({name:e[1].replace(/\s+/g," ").trim(),slug:e[2]})).filter(e=>"create"!==e.slug&&e.name&&!/^more$/i.test(e.name));if(!r.length)return t;let n=N(i),o=e=>{let t=new Set(N(e));return n.filter(e=>t.has(e)).length/Math.max(n.length,1)},s=new Map;for(let e of r){let t=R(e.name),i=Math.max(o(e.slug),o(e.name)),a=s.get(t);a?(a.slugs.push(e.slug),a.overlap=Math.max(a.overlap,i)):s.set(t,{name:e.name,slugs:[e.slug],overlap:i})}let l=[...s.values()].sort((e,t)=>t.overlap-e.overlap),c=l.filter(e=>e.overlap>=.5),d=c.length?c:l,u=d.slice(0,5).map(e=>({name:e.name,slug:e.slugs[0],overlap:e.overlap})),m=d.find(e=>R(e.name)===R(i));if(!m&&d.length>=2&&d[0].overlap-d[1].overlap<.2)return{chosen:null,distinct:u,ambiguous:!0};let p=m??d[0];if(!m){let e=(i.toLowerCase().match(/[a-z0-9]+/g)||[]).find(e=>e.length>=3&&!A.has(e))||"",t=new Set([...N(p.name),...p.slugs[0].split("-")]),a=!e||t.has(e);if(p.overlap<.2||!a)return{chosen:null,distinct:u,ambiguous:!1}}let h=null;for(let e of p.slugs.slice(0,2)){let t=await I(`https://incidecoder.com/products/${e}`);if(!t)continue;let i=function(e,t){let i=e.indexOf("## Ingredients overview");if(-1===i)return null;let a=e.slice(i,i+8e3).split("\n").map(e=>e.trim()).find(e=>(e.match(/\/ingredients\//g)||[]).length>=3);if(!a)return null;let r=[...a.matchAll(/\[([^\]]+)\]\([^)]*\/ingredients\/[^)]*\)/g)].map(e=>e[1].replace(/​/g,"").replace(/\s+/g," ").trim()).filter(e=>e&&"more"!==e.toLowerCase());if(r.length<3)return null;let n=[];/alcohol[\s-]?free/i.test(e)&&n.push("alcohol-free"),/fragrance[\s&]*(?:and\s+)?(?:essential[\s-]?oil[\s-]?)?free/i.test(e)&&n.push("fragrance & essential-oil free");let o=e.match(/Title:\s*(.+?)\s+ingredients/i),s=o?o[1].trim():t.replace(/-/g," "),l=e.match(/https:\/\/incidecoder-content[^\s)"']+?(?:photo|front)[^\s)"']*\.(?:jpe?g|png|webp)/i);return{productName:s,slug:t,ingredients:r,tags:n,source:`https://incidecoder.com/products/${t}`,imageUrl:l?l[0]:null}}(t,e);i&&(!h||i.ingredients.length>h.ingredients.length)&&(h=i)}return{chosen:h,distinct:u,ambiguous:!1}}var E=e.i(77465),T=e.i(13452);let O="TCS v3.0",P=e=>/^https?:\/\//i.test(e.trim());function D(e){if(!e||"object"!=typeof e||"product-review"!==e.type||"string"!=typeof e.productName||!e.productName||!Array.isArray(e.claimMap))return!1;let t=e.scores;return!!t&&"number"==typeof t.total}function L(e){let t=e.scores?.claimEvidence??0,i=e.scores?.formulaLogic??0,a=e.scores?.total??0,r=e.formulaLogic?.claimOverreach===!0,n=/spf|pa\+|uva|uvb|broad.?spectrum|sun.?protection|blue light/i,o=/\b(cures?|treats?|heals?|whitens?|whitening|fairness|lightens?\s+skin|permanent(?:ly)?|guaranteed?)\b|contradict|not listed in|inci lists|own ingredient/i,s=(e.claimMap??[]).filter(e=>{let t=`${e.text} ${e.evidenceNote??""} ${e.asciNote??""} ${e.drugBoundaryNote??""}`;return!!(e.drugBoundaryRisk&&!n.test(e.text)||"red-flag"===e.riskLevel&&o.test(t))}).length,l=0===s,c=t>=10,d=r&&i<11,u=i>=8&&!d,m=[{id:"claims",label:"Lawful claims",passed:l,detail:l?"No drug-boundary or unlawful claims found":`${s} claim(s) cross the India drug-cosmetic boundary or are contradicted`},{id:"evidence",label:"Honest evidence",passed:c,detail:c?"Claims are substantially backed by visible evidence":"Most claims rely on borrowed ingredient-level evidence, not finished-product proof"},{id:"formula",label:"Formula supports claims",passed:u,detail:u?r?"The formula delivers the core claims; one claim is noted as ambitious":"The formula and format plausibly deliver the claims":d?"Claims go beyond what this formula/format can plausibly deliver":"Formula logic is weak relative to the claims made"}],p=s>0?"misleading":a>=85&&t>=15?"approved":a>=65?"mostly-clean":"needs-proof",h={approved:{label:"Clean Sheet Approved",headline:"Claims hold up to the evidence."},"mostly-clean":{label:"Mostly Clean",headline:"A well-made product, but some claims rest on ingredient evidence, not finished-product proof."},"needs-proof":{label:"Needs Proof",headline:"The claims may be honest, but the proof isn't publicly visible yet."},misleading:{label:"Misleading Claims",headline:"Makes claims that aren't permitted or are contradicted by its own ingredient list."}};return{status:"approved"===p?"approved":"not_approved",tier:p,tierLabel:h[p].label,headline:h[p].headline,gates:m,standard:`${O} \xb7 claims \xb7 evidence \xb7 formula`}}let x=new Map;function M(e){return"product-review"===e.type?{...e,verdict:L(e.review)}:e}async function F(e){let t=x.get(e);if(t)return M(t);try{let{data:t}=await (0,T.createAdminClient)().from("product_reviews").select("result, rubric_rev").eq("product_slug",e).maybeSingle();if(t?.result&&"r5"===t.rubric_rev){let i=t.result;return x.set(e,i),M(i)}}catch{}return null}async function U(e,t){if(x.set(e,t),"product-review"===t.type)try{await (0,T.createAdminClient)().from("product_reviews").upsert({product_slug:e,product_name:t.review.productName,brand:t.review.brand,tier:t.verdict.tier,image_url:t.review.imageUrl??null,result:t,rubric_rev:"r5",reviewed_at:t.review.reviewedAt??new Date().toISOString()},{onConflict:"product_slug"})}catch{}}async function H(e){var t;let i=e.trim();if(!i)return{type:"error"};let a=null,r="",n="",o=null;if(P(i)){let e=await I(i);if(e){let t;r=C(e),(o=(t=e.match(/^Title:\s*(.+)$/m))&&t[1].replace(/^buy\s+/i,"").replace(/\s*(online.*|at best price.*)$/i,"").split("|")[0].split("–")[0].trim()||null)&&(a=(await k(o)).chosen);let s=function(e,t,i=3){let a="";try{a=new URL(t).hostname.replace(/^www\./,"")}catch{}let r=new Set,n=[];for(let o of e.matchAll(/\[([^\]]*)\]\((https?:\/\/[^)\s]+)\)/g)){let[,e,s]=o;if(!r.has(s)){if(r.add(s),S.test(e)||S.test(s)){try{let e=new URL(s).hostname.replace(/^www\./,"");if(/facebook|instagram|twitter|youtube|linkedin|wa\.me|whatsapp|apple|google/.test(e)||a&&e!==a&&!e.includes(a.split(".")[0]))continue}catch{continue}if(s!==t&&(n.push(s),n.length>=i))break}}}return n}(e,i),l=[];for(let e of s.slice(0,2)){let t=await I(e,2e4);t&&l.push(`SOURCE: ${e}
${C(t,4e3)}`)}l.length&&(n=`
...
${l.join("\n---\n")}`)}}else{let e=await k(i);if(e.ambiguous)return{type:"disambiguation",query:i,options:e.distinct.map(e=>({name:e.name}))};a=e.chosen}let s=a?.slug??(0,E.slugify)(P(i)?o??i:i,""),l=await F(s);if(l)return l;let c=(t=a)&&t.ingredients.length?`

RETRIEVED INGREDIENT LIST for "${t.productName}" (from INCIDecoder, ${t.source}). Use it as follows:
...
INGREDIENTS: ${t.ingredients.join(", ")}${t.tags.length?`
INCIDecoder labels this product: ${t.tags.join("; ")}.`:""}`:`

INGREDIENT LIST: could not be retrieved automatically. Do NOT assert which ingredients are present or absent from memory — search for the INCI, and if you still cannot confirm it, mark ingredient-dependent claims as unverified.`,d=a?`
The product under review is exactly: "${a.productName}". Use this exact identity as productName/brand in your output. Do NOT substitute a different size or variant, and do NOT review a different product than this one.`:"",u=r?`

PRODUCT PAGE CONTENT (scraped from the URL the user provided — treat as the primary source of the product's claims):
${r}`:"",m=P(i)?`Produce a full Clean Sheet Product Review for the product at this URL: ${i}${d}
Also search Nykaa, Amazon.in, Flipkart and the brand site for its price, claims, and ingredient list.${u}${n}${c}`:`Produce a full Clean Sheet Product Review for this product: ${i}${d}
Search its official page and Nykaa, Amazon.in, Flipkart and quick-commerce listings for price, claims, and the ingredient list.${c}`,p=null;for(let e of[m,`${m}

Return ONLY the product-review JSON, starting directly with { and ending with }. No prose, no code fence, no trailing commas.`,`${m}

Your previous output was not valid JSON. Return ONLY the complete, valid product-review JSON object.`]){if(p=function(e){if(!e)return null;let t=e.trim().replace(/^```(?:json)?/i,"").replace(/```$/,"").trim(),i=t.indexOf("{"),a=t.lastIndexOf("}");if(-1===i||-1===a||a<i)return null;t=t.slice(i,a+1).replace(/,(\s*[}\]])/g,"$1");try{return JSON.parse(t)}catch{return null}}(await (0,b.generateResilient)(v,e)),p?.type==="out_of_scope")return{type:"out_of_scope"};if(D(p))break}if(!D(p))return{type:"error"};let h=p,g=a?.imageUrl??null;if(!g&&P(i)&&(g=await (0,w.resolveProductImage)(i)),g||(g=await (0,w.findProductImageKeyless)(h.brand,h.productName)),!g){let e=[h.brand,h.productName].filter(Boolean).join(" ");g=await (0,w.searchProductImage)(e||i,h.brand)}h.imageUrl=g,h.methodologyVersion=O,h.reviewedAt=new Date().toISOString(),h.productSlug=s,a&&(h.inciIngredients=a.ingredients,h.inciSourceUrl=a.source);let f=L(h);"approved"===f.status&&await (0,E.upsertVerifiedProduct)({slug:(0,E.slugify)(h.productName,h.brand),productName:h.productName,brand:h.brand,score:h.scores.total,scoreLabel:h.scores.label,integrityScore:null,imageUrl:h.imageUrl??null,summary:h.verdict?.cleanSheetTakeaway||h.priceInsight||"",usageGuidance:null,verifiedAt:h.reviewedAt||new Date().toISOString(),methodologyVersion:O});let y={type:"product-review",review:h,verdict:f};return await U(s,y),y}var B=e.i(89104);async function q(e){try{if(!(0,B.rateLimit)(e,"review",6))return(0,B.rateLimited)();let{query:t}=await e.json();if(!t?.trim())return Response.json({error:"No query provided"},{status:400});let i=await H(String(t));return Response.json(i)}catch(e){if(console.error("[review]",e instanceof Error?e.message:e),e instanceof b.TransientModelError)return(0,b.busyResponse)();return Response.json({type:"error"},{status:500})}}e.s(["POST",0,q,"dynamic",0,"force-dynamic","maxDuration",0,180],73448);var z=e.i(73448);let _=new t.AppRouteRouteModule({definition:{kind:i.RouteKind.APP_ROUTE,page:"/api/review/route",pathname:"/api/review",filename:"route",bundlePath:""},distDir:"/tmp/tcs-next",relativeProjectDir:"",resolvedPagePath:"[project]/src/app/api/review/route.ts",nextConfigOutput:"",userland:z,...{}}),{workAsyncStorage:V,workUnitAsyncStorage:$,serverHooks:j}=_;async function Y(e,t,a){a.requestMeta&&(0,r.setRequestMeta)(e,a.requestMeta),_.isDev&&(0,r.addRequestMeta)(e,"devRequestTimingInternalsEnd",process.hrtime.bigint());let v="/api/review/route";v=v.replace(/\/index$/,"")||"/";let b=await _.prepare(e,t,{srcPage:v,multiZoneDraftMode:!1});if(!b)return t.statusCode=400,t.end("Bad Request"),null==a.waitUntil||a.waitUntil.call(a,Promise.resolve()),null;let{buildId:w,deploymentId:I,params:C,nextConfig:S,parsedUrl:N,isDraftMode:A,prerenderManifest:R,routerServerContext:k,isOnDemandRevalidate:E,revalidateOnlyGenerated:T,resolvedPathname:O,clientReferenceManifest:P,serverActionsManifest:D}=b,L=(0,s.normalizeAppPath)(v),x=!!(R.dynamicRoutes[L]||R.routes[O]),M=async()=>((null==k?void 0:k.render404)?await k.render404(e,t,N,!1):t.end("This page could not be found"),null);if(x&&!A){let e=!!R.routes[O],t=R.dynamicRoutes[L];if(t&&!1===t.fallback&&!e){if(S.adapterPath)return await M();throw new f.NoFallbackError}}let F=null;!x||_.isDev||A||(F="/index"===(F=O)?"/":F);let U=!0===_.isDev||!x,H=x&&!U;D&&P&&(0,o.setManifestsSingleton)({page:v,clientReferenceManifest:P,serverActionsManifest:D});let B=e.method||"GET",q=(0,n.getTracer)(),z=q.getActiveScopeSpan(),V=!!(null==k?void 0:k.isWrappedByNextServer),$=!!(0,r.getRequestMeta)(e,"minimalMode"),j=(0,r.getRequestMeta)(e,"incrementalCache")||await _.getIncrementalCache(e,S,R,$);null==j||j.resetRequestCache(),globalThis.__incrementalCache=j;let Y={params:C,previewProps:R.preview,renderOpts:{experimental:{authInterrupts:!!S.experimental.authInterrupts},cacheComponents:!!S.cacheComponents,supportsDynamicResponse:U,incrementalCache:j,cacheLifeProfiles:S.cacheLife,waitUntil:a.waitUntil,onClose:e=>{t.on("close",e)},onAfterTaskError:void 0,onInstrumentationRequestError:(t,i,a,r)=>_.onRequestError(e,t,a,r,k)},sharedContext:{buildId:w,deploymentId:I}},G=new l.NodeNextRequest(e),W=new l.NodeNextResponse(t),X=c.NextRequestAdapter.fromNodeNextRequest(G,(0,c.signalFromNodeResponse)(t));try{let r,o=async e=>_.handle(X,Y).finally(()=>{if(!e)return;e.setAttributes({"http.status_code":t.statusCode,"next.rsc":!1});let i=q.getRootSpanAttributes();if(!i)return;if(i.get("next.span_type")!==d.BaseServerSpan.handleRequest)return void console.warn(`Unexpected root span type '${i.get("next.span_type")}'. Please report this Next.js issue https://github.com/vercel/next.js`);let a=i.get("next.route");if(a){let t=`${B} ${a}`;e.setAttributes({"next.route":a,"http.route":a,"next.span_name":t}),e.updateName(t),r&&r!==e&&(r.setAttribute("http.route",a),r.updateName(t))}else e.updateName(`${B} ${v}`)}),s=async r=>{var n,s;let l=async({previousCacheEntry:i})=>{try{if(!$&&E&&T&&!i)return t.statusCode=404,t.setHeader("x-nextjs-cache","REVALIDATED"),t.end("This page could not be found"),null;let n=await o(r);e.fetchMetrics=Y.renderOpts.fetchMetrics;let s=Y.renderOpts.pendingWaitUntil;s&&a.waitUntil&&(a.waitUntil(s),s=void 0);let l=Y.renderOpts.collectedTags;if(!x)return await (0,m.sendResponse)(G,W,n,Y.renderOpts.pendingWaitUntil),null;{let e=await n.blob(),t=(0,p.toNodeOutgoingHttpHeaders)(n.headers);l&&(t[g.NEXT_CACHE_TAGS_HEADER]=l),!t["content-type"]&&e.type&&(t["content-type"]=e.type);let i=void 0!==Y.renderOpts.collectedRevalidate&&!(Y.renderOpts.collectedRevalidate>=g.INFINITE_CACHE)&&Y.renderOpts.collectedRevalidate,a=void 0===Y.renderOpts.collectedExpire||Y.renderOpts.collectedExpire>=g.INFINITE_CACHE?void 0:Y.renderOpts.collectedExpire;return{value:{kind:y.CachedRouteKind.APP_ROUTE,status:n.status,body:Buffer.from(await e.arrayBuffer()),headers:t},cacheControl:{revalidate:i,expire:a}}}}catch(t){throw(null==i?void 0:i.isStale)&&await _.onRequestError(e,t,{routerKind:"App Router",routePath:v,routeType:"route",revalidateReason:(0,u.getRevalidateReason)({isStaticGeneration:H,isOnDemandRevalidate:E})},!1,k),t}},c=await _.handleResponse({req:e,nextConfig:S,cacheKey:F,routeKind:i.RouteKind.APP_ROUTE,isFallback:!1,prerenderManifest:R,isRoutePPREnabled:!1,isOnDemandRevalidate:E,revalidateOnlyGenerated:T,responseGenerator:l,waitUntil:a.waitUntil,isMinimalMode:$});if(!x)return null;if((null==c||null==(n=c.value)?void 0:n.kind)!==y.CachedRouteKind.APP_ROUTE)throw Object.defineProperty(Error(`Invariant: app-route received invalid cache entry ${null==c||null==(s=c.value)?void 0:s.kind}`),"__NEXT_ERROR_CODE",{value:"E701",enumerable:!1,configurable:!0});$||t.setHeader("x-nextjs-cache",E?"REVALIDATED":c.isMiss?"MISS":c.isStale?"STALE":"HIT"),A&&t.setHeader("Cache-Control","private, no-cache, no-store, max-age=0, must-revalidate");let d=(0,p.fromNodeOutgoingHttpHeaders)(c.value.headers);return $&&x||d.delete(g.NEXT_CACHE_TAGS_HEADER),!c.cacheControl||t.getHeader("Cache-Control")||d.get("Cache-Control")||d.set("Cache-Control",(0,h.getCacheControlHeader)(c.cacheControl)),await (0,m.sendResponse)(G,W,new Response(c.value.body,{headers:d,status:c.value.status||200})),null};V&&z?await s(z):(r=q.getActiveScopeSpan(),await q.withPropagatedContext(e.headers,()=>q.trace(d.BaseServerSpan.handleRequest,{spanName:`${B} ${v}`,kind:n.SpanKind.SERVER,attributes:{"http.method":B,"http.target":e.url}},s),void 0,!V))}catch(t){if(t instanceof f.NoFallbackError||await _.onRequestError(e,t,{routerKind:"App Router",routePath:L,routeType:"route",revalidateReason:(0,u.getRevalidateReason)({isStaticGeneration:H,isOnDemandRevalidate:E})},!1,k),x)throw t;return await (0,m.sendResponse)(G,W,new Response(null,{status:500})),null}}e.s(["handler",0,Y,"patchFetch",0,function(){return(0,a.patchFetch)({workAsyncStorage:V,workUnitAsyncStorage:$})},"routeModule",0,_,"serverHooks",0,j,"workAsyncStorage",0,V,"workUnitAsyncStorage",0,$],15359)}];

//# sourceMappingURL=node_modules_next_dist_esm_build_templates_app-route_0y5tbzn.js.map